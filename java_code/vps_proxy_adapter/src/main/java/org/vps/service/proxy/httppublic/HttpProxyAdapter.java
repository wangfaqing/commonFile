...
import org.vps.service.proxy.PublicAuthorizationCheckForHttp;
...
//        return true;
	...
	private ByteBuf CreateSendBuffer(HttpRequestHeader recvHeader) {
		...
		String strProxyAuthHeaderLine = "Proxy-Authorization: Basic " + Base64.encodeToString(PROXY_USER + ":" + ProxyNewPwd.build(PROXY_KEY, Integer.toString(proxyParam.getRid()), proxyParam.getUid(), proxyParam.getPid(), proxyParam.getSip(), -1));
...
//	        if (publicUser != null && isCountOfConn) {
//	            AppInstance.getPublicUserOrderMgr().DisconnPublicUser(publicUser);
//	        }
//	        if (publicUser != null && logProxySession.getRecvByteCounts() + logProxySession.getSendByteCounts() > 0) {
//	            AppInstance.getPublicUserOrderMgr().updatePublicUserFlow(publicUser, logProxySession.getRecvByteCounts() + logProxySession.getSendByteCounts());
//	        }
			release();
		} catch (Exception e) {
...
//            }
            ...
            //proxyParam
            // 选择代理服务器进行连接
            String hostPort = ProxyTransferRule.get(proxyParam.getUser(), proxyParam.getRid() + "_" + proxyParam.getUid());
            if (!StringUtil.isBlank(hostPort)) {
                ...
                ProxyTransferRule.set(proxyParam.getUser(), proxyParam.getRid() + "_" + proxyParam.getUid(), hostPort);
                ...
                ProxyTransferRule.set(proxyParam.getUser(), proxyParam.getRid() + "_" + proxyParam.getUid(), keyValue.getKey()+":"+keyValue.getValue());
